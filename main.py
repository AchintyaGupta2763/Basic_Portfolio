...
if option == 'About Me':
    ...
    st.markdown("<h1 style='text-align: left; font-size: 120px; color: white;'>ACHINTYA GUPTA</h1>", unsafe_allow_html=True)
    ...
    with tab1:
        ...
        with col2:
            multi = ['Phone number = +91 9452438229', 'EmailID = achintyagupta1@gmail.com',
                     'LinkedinID =  www.linkedin.com/in/achintya-gupta-896311250/',
                     'Github id = https://github.com/AchintyaGupta2763', 'Location = Varanasi, Uttar Pradesh']

            for i in multi:
                st.subheader(i)
    with tab2:
        st.title("Summary")
        st.subheader('I am a second-year BTech student with a passion for Artificial intelligence and Machine Learning.'
                      ' Proficient in Python and C++, I am eager to learn & understand new things and collaborate with professionals to contribute in real-world projects.')
...
if option == 'Experience and Project':
    ...
    with st.container():
        ...
        with col1:
            st.header('Deep Fake Image Detection:')
            st.subheader('Helps to identify whether the image is fake or real')
            st.text('')
            st.text('')
            st.header('Damage Detection using YOLO v4:')
            st.subheader('Helps to detect any absurd or unique things (i.e Damage) in an oject')
            st.text('')
            st.text('')
            ...
            st.subheader('An air quality predictor and solution provider that gives results on receiving other parameters')
